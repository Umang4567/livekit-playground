  ...
  const settingsTileContent = useMemo(() => {
    return (
      <div className="flex flex-col h-full w-full items-start overflow-y-auto">
        <ConfigurationPanelItem title="Prompt">
          <textarea
            className="w-full min-h-[80px] p-2 rounded border border-gray-700 bg-gray-900 text-gray-100 focus:outline-none focus:ring-2 focus:ring-cyan-500 resize-vertical"
            placeholder="Enter the prompt for the bot to use in the backend..."
            value={config.settings.prompt || ""}
            onChange={e => {
              const newSettings = { ...config.settings, prompt: e.target.value };
              setUserSettings(newSettings);
            }}
          />
        </ConfigurationPanelItem>

        <ConfigurationPanelItem title="First Message">
          <textarea
            className="w-full min-h-[60px] p-2 rounded border border-gray-700 bg-gray-900 text-gray-100 focus:outline-none focus:ring-2 focus:ring-cyan-500 resize-vertical"
            placeholder="Enter the first message to send to the agent..."
            value={config.settings.firstMessage || ""}
            onChange={e => {
              const newSettings = { ...config.settings, firstMessage: e.target.value };
              setUserSettings(newSettings);
            }}
          />
        </ConfigurationPanelItem>
