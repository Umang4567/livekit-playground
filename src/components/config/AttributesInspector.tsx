...
export const AttributesInspector: React.FC<AttributesInspectorProps> = ({
  ...
  // Find current TTS/STT model from attributes
  const ttsModel = localAttributes.find((a) => a.key === "tts_model")?.value || "";
  const sttModel = localAttributes.find((a) => a.key === "stt_model")?.value || "";

  const handleTTSModelChange = (value: string) => {
    let updated = false;
    const updatedAttributes = localAttributes.map((attr) => {
      if (attr.key === "tts_model") {
        updated = true;
        return { ...attr, value };
      }
      return attr;
    ...
    if (!updated) {
      updatedAttributes.push({ id: `tts_model_${Date.now()}`, key: "tts_model", value });
    }
    setLocalAttributes(updatedAttributes);
    onAttributesChange(updatedAttributes);
    if (connectionState === ConnectionState.Connected) {
  ...
  const handleSTTModelChange = (value: string) => {
    ...
    if (!updated) {
      updatedAttributes.push({ id: `stt_model_${Date.now()}`, key: "stt_model", value });
    }
    setLocalAttributes(updatedAttributes);
    onAttributesChange(updatedAttributes);
    if (connectionState === ConnectionState.Connected) {
    ...
    <div>
      ...
      {isExpanded && (
        <div className="border border-gray-800 rounded-sm bg-gray-900/30 p-3 mb-2">
          {/* TTS Model Dropdown */}
          <div className="mb-2">
            <label className="block text-xs text-gray-400 mb-1">TTS Model</label>
            <select
